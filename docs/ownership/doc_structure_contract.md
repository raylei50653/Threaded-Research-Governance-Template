...
**Series:** O-series companion (**O1.5**)  
...
| Decision-layer *why* (policy / gates / knobs) | `docs/research/decision-log/` (or project name) | **Closed line is read-only**; reopen only as a *named* new line with evidence |
...
| Deep **per-signal / per-gate** analysis progress | `docs/research/eval/signal_analysis_ledger.md` | One row per `signal_id`; numbers master = `out/` (or project study dir) |
...
| Stable module design | `docs/modules/<m>/architecture*.md` | Link from module README “設計入口” |
...
1. **Card** — 職責 / 現況 / I/O / GO·NO-GO.
2. **Design entry** — `architecture*` and/or “see decision-log / ADR …”.
3. **Research index** (if `research/` exists) — **list every** `research/*.md` with status; put `🔄 Active` first.
4. **TODO link** — `TODO.md` only.

Copy from [docs/modules/_template/](../modules/_template/).
...
- `docs/research/**/*.md` (dated notes; not pure index READMEs)
...
```html
<!-- doc-module: module-a | module-b | cross | … -->
```
...
| Entry | Obligation |
|:--|:--|
| `docs/research/README.md` | Active workstreams, Closed lines, Paper → `report_data`, subdir entry points（含 `threads/`）; **no phantom paths** |
| `docs/research/threads/README.md` | Index **all** active thread cards; threads are navigation-only |
| `docs/research/<sub>/README.md` | Index **all** `.md` in that subdir (except the README itself), **or** state “no index; filenames only” |
| `docs/modules/<m>/README.md` | If `research/` exists, index **all** research notes |
| `report_data/README.md` | Start-here list; **one-line** link to decision paper outline (if any) |

**Same-PR rule:** adding a research file **must** add the matching README index row.

Checker (phase 2): `scripts/tools/check_doc_structure.py` (warn-only) flags research notes not mentioned by the owning README.
...
| Line | Owner | Use |
|:--|:--|:--|
| Decision / production narrative | e.g. `docs/research/paper_outline.md` + `evidence_ledger.md` | System / product story |
| Method paper assets | `report_data/` (+ optional `paper_direction.md`) | Method thesis / tables / figures |
...
**Closed decision line:** keep read-only; no drive-by reopen (WIP rules). Reopen only as a *named* new line with evidence.
...
| Machine (phase 2) | `check_doc_structure.py` **warn-only** — research file not referenced by owning README |

```bash
# phase 2
# python3 scripts/tools/check_doc_structure.py
```
...
- Module README links design + research + decision-log (if any).
- Paper outline ↔ `report_data/README.md` mutual links (when both exist).
...
- Topic hubs
- Tighten checker to `--strict` after index debt is paid down
